...
proza_tirgum = [
    ...
    [{"יהודים": []}],
    [{"רומן היסטורי אמריקאי": ["היסטוריה אמריקאי", "ההיסטוריה האמר", "ההיסטוריה של ארה"]}],
    [{"ספרות אפרו-אמריקאי": ["אפרו אמריקאי", "אפרו-אמריקאי", "שחורים", "שחורה", "הפרדה גז", "ההפרדה הגז", "כושי",
                             "ג'ים קרואו"]}],
    [{"ספרות סינית": ["סינית", "של סין", "בסין", "שנחאי", "בייג'ינ", "סצ'ואן"]}],
    [{"ספרות יפנית": ["יפני", "יפן", "טוקיו", "קיוטו"]}],
    [{"ספרות ערבית": ["ערבי"]}],
    [{"ספרות דרום אמריקאית": ["דרום אמרי", "קה הלטיני", "דרום-אמר", "ארגנטינ", "בוליבי", "אקוודור", "ברזיל", "גואטמל",
                              "האיטי", "קוסטה ר", "ונצואל", "קולומבי", "מקסיקו", "קובה", "צ'ילה"]}],
    [{"רומן היסטורי בריטי": ["היסטוריה בריט", "קה הלטיני", "היסטוריה סק", "ההיסטוריה הב", "ההיסטוריה הסק",
                             "האימפריה הברי", "קולוניאלי", "הויקטוריאנ", "בית המלוכה הבר", "מלכת אנג",
                             "אנגליה של המאה ה", "לונדון של המאה ה", "אבירים", "חצר המלוכה", "הצי המלכותי"]}],
    [{"ספרות פורטוגלית": ["פורטוגל", "ליסבון", "סאראמאגו", "שנחאי"]}],
    [{"ספרות נורדית": ["איסלנדית", "דנית", "נורווגית", "שוודית", "סקנדינב"]}],
    [{"ספרות קנדית": ["קנדי", "מרגרט אטווד", "קוויבק", "טורונטו", "מונטריאול", "אי הנסיך אד", "ונקובר", "ויניפג",
                      "אונטריו"]}],
    [{"ספרות איטלקית": ["הספרות האיט", "ספרות נפוליטינ", "הספרות הנפ", "ספרות סיציליאנ", "הספרות הסיצי", "אלנה פרנטה",
                        "של איטליה", "נטליה גינצבורג", "פאולו ג'ורדנו"]}],
...
eyuin = [
    ...
    [{"אסטרונומיה": []}],
    [{"משמעות החיים": []}],
    [{"מחול": ["ריקוד", "כוריאוגרפ"]}],
    [{"פוסט מודרניזם": ["פוסטמוד", "פוסט-מוד", "פוסט מודר"]}],
    [{"קבצי מכתבים": ["מכתבים", "התכתבויות", "חילופי דוא"]}],
    [{"פמיניזם": ["פמיניס", "פמיניס", "זכויות נשים"]}],
    [{"תיאוריה מוזיקלית": ["תיאוריה של המוסי", "תיאוריה מוסיק", "תיאוריה של המוז"]}],
    [{"פילוסופיה": ["פילוסופיית", "תורת ה", "ההגות"]}],
    [{"מדע ורפואה": ["מדע", "רפוא", "ביולוגיה"]}],
    [{"אדריכלות": ["ארכיטקט", "תכנון עירוני", "תכנון ער"]}],
    [{"לשון": ["בלשנות", "לינגוויסט", "שפה"]}],
    [{"חוק ומשפט": ["משפטי", "חקיקה", "חוקים"]}],
    [{"פמיניזם": ["פמיניס", "פמיניס", "זכויות נשים"]}],
    [{"פמיניזם": ["פמיניס", "פמיניס", "זכויות נשים"]}],
    [{"פמיניזם": ["פמיניס", "פמיניס", "זכויות נשים"]}],
    [{"מגדר": []}],
    ...
    [{"דקונסטרוקציה": []}],
    [{"מיתולוגיה": []}],
    [{"סבתאות וסבאות": ["סבתאות", "סבאות", "להיות סבתא", "להיות סבא"]}],
    [{"שיחות": []}],
    [{"ביולוגיה": []}],
    [{"מעוררת מחשבה": ["מעורר מחשבה"]}],
    [{"ציונות הדתית": ["ציונות דתית"]}],
    [{"אבולוציה": []}],
    [{"מדעים": []}],
    [{"צילום": []}],
    [{"אתיקה": ["מוסר"]}],
    ...
    [{"תיעודי": ["דוקומנטר"]}],
    [{"מדע פופולארי": []}],
    [{"אנתרופולוגיה": ["אנתרופול"]}],
    [{"סוציולוגיה": ["סוציולו"]}],
    [{"איסלאם": ["איסלם", "מוסלמי", "איסלמ"]}],
    [{"מדעי המוח": ["נוירו"]}],
    [{"תקשורת": []}],
    [{"קפיטליזם": []}],
    [{"ארץ ישראל": []}],
...
fantasy = [
    ...
    [{"קלאסיקת מדע בדיוני": ["קלאסי"]}],
                             [{"אופרת חלל": []}],
                             [{"סדרת משחקי הכס (שיר של אש ושל קרח)": ["שיר של אש", "משחקי הכס"]}],
                             [{"רובוטים": ["רובוט"]}],
                             [{"קסם וכישוף": ["קוסם", "קוסמ", "מכשף", "מכשפ"]}],
                             [{"דרקונים": ["דרקו"]}],
                                           [{"מכשפות": []}],
                                           [{"ערפדים": ["ערפד"]}],
                                           [{"מותחן מדע בדיוני": []}],
...
children_and_teenagers = [
    ...
    [{"דמיון וקסם": ["קוסמ", "קוסם", "דימיונ", "דמיונ",
                     "דימיון", "דמיון", "קסם"]}],
    [{"בעלי חיים לילדים": ["חיות", "כלב", "חתול", "סוס", "פרה",
                           "עכבר", "סנאי", "ציפור", "נחש",
                           "כבש", "עז", "תרנגול", "אפרוח",
                           "פיל", "תנין", "אריה", "קיפוד",
                           "שועל", "קרנף", "קוף", "זיקית", "דג",
                           "לויתן", "תמנון", "לביאה", "זברה",
                           "נמר", "דוב", "ג'ירפה",
                           "תוכי" "ארנב", "שפן", "בעל חיים"]}],
    [{"לילדים ולמבוגרים": ["לילדים בלבם", "ילדים בנפשם",
                           "וגם למבוגרים"]}],
    [{"בלשים צעירים": ["בלש"]}],
    ...
    [{"לידה ואח קטן": ["לידה", "היריון", "אח קטן", "אחות קטנה",
                       "בטן של אמא", "הריון"]}],
    ...
    [{"ילדי גן": ["בגן", "גן הילד", "גן ילד", "גן חובה",
                  "טרום חובה", "גנון", "פעוטון", "לגן"]}],
    [{"פנטזיה צעירה": ["פנטזיה"]}],
    [{"תינוקות": ["תינוק"]}],
    [{"נוער בוגר": ["נוער"]}],
...
history_and_politics = [
    ...
    [{"מלחמות ישראל": ["מלחמת יום", "צוק איתן", "חומת מגן", "מלחמת הש", "מלחמת המ", "מלחמת ששת", "מלחמת ל", "מבצע סי",
                       "מבצע של", "מבצע סי", "1973", "1967", "1948"]}],
...
psychology = [
    ...
    [{"אושר": []}],
    [{"פסיכיאטריה": ["פסיכיאטר"]}],
    [{"צמיחה ממשבר": ["משבר", "צמיחה",
                      "לצמוח"]}],
    [{"טראומה - התמודדותלד וטיפול": ["טראומ",
                                     "PTSD"]}],
    [{"פסיכולוגיית ילדים": ["של הילד",
                            "טיפול בי"]}],
    [{"OCD": ["כפייתי", "או-סי-די"]}],
    [{"ADHD": ["קשב וריכוז", "ADD"]}],
    [{"לקויות למידה": ["לקות למ", "דיסקלק",
                       "דיסלק"]}],
    [{"התמכרות": ["אלכוהוליזם", "סמים",
                  "גמילה"]}],
    [{"צרכים מיוחדים": ["נכות", "פיגור",
                        "תסמונת", "מוגבלות"]}],
    [{"חרדה": ["חרד", "פובי"]}],
    [{"דיכאון": ["דיסתמי", "מדוכא"]}],
